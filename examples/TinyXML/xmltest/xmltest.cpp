...
int main()
{
		...
		
	{
...

		{
			...
			TiXmlDocument doc( "../xml_files/demotest.xml" );
			doc.Parse( demoStart );
		...
		if ( !loadOkay )
		{
			printf( "Could not load test file '../xml_files/demotest.xml'. Error='%s'. Exiting.\n", doc.ErrorDesc() );
			exit( 1 );
		}
	...
	printf ("\n** UTF-8 **\n");
	{
		TiXmlDocument doc( "../xml_files/utf8test.xml" );
		doc.LoadFile();
		if ( doc.Error() && doc.ErrorId() == TiXmlBase::TIXML_ERROR_OPENING_FILE ) {
			printf( "WARNING: File '../xml_files/utf8test.xml' not found.\n"
		...
		else
		{
			...
			doc.SaveFile( "../xml_files/utf8testout.xml" );
			...
			FILE* saved  = fopen( "../xml_files/utf8testout.xml", "r" );
			FILE* verify = fopen( "../xml_files/utf8testverify.xml", "r" );
...

	{
		...
		TiXmlDocument doc( "../xml_files/passages.xml" );
		doc.Parse( passages );
    ...
    {
		FILE* textfile = fopen( "../xml_files/test5.xml", "w" );
		if ( textfile )
		{
			...
			TiXmlDocument doc;
            doc.LoadFile( "../xml_files/test5.xml" );
            XmlTest( "dot in element attributes and names", doc.Error(), 0);
		}
    }

	{
		FILE* textfile = fopen( "../xml_files/test6.xml", "w" );
		if ( textfile )
		{
            ...
            TiXmlDocument doc;
            bool result = doc.LoadFile( "../xml_files/test6.xml" );
...

	{
		...
		TiXmlDocument doc;
		doc.Parse( doctype );
		doc.SaveFile( "../xml_files/test7.xml" );
		doc.Clear();
		doc.LoadFile( "../xml_files/test7.xml" );
...

	{
		...
		/* Could not repro. {
			TiXmlDocument xml;
			xml.LoadFile( "../xml_files/EQUI_Inventory.xml" );
